...
void processYOLODetections(const float* output_data, const std::vector<int64_t>& shape, 
    ...
    std::vector<cv::Scalar> colors = {
        ...
        cv::Scalar(128, 0, 128),  cv::Scalar(255, 165, 0),  cv::Scalar(128, 128, 0),
        cv::Scalar(0, 128, 128)
    };
...
void processVideo(const std::string& video_path, std::string url, bool verbose, tc::Headers http_headers) {
   ...
   // cv::resizeWindow("YOLO Object Detection - Triton Server", 800, 600);
    ...
    while (cap.read(frame) && frame_count < max_frames) {
        ...
        double elapsed_ms = std::chrono::duration<double, std::milli>(now - prev_time).count();
        double current_fps = (elapsed_ms > 0) ? 1000.0 / elapsed_ms : 0.0;
        prev_time = now;
...
cv::waitKey(30);
    ...
    if (frame_count > 0) {
        ...
        std::cout << "FPS promedio: " << (1000.0 / (total_inference_time / frame_count)) << std::endl;
    }
