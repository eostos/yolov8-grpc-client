...
cv::Rect Yolo::get_rect_2(const cv::Size& imgSz, const std::vector<float>& bbox)
{
    ...
    std::cout << "Input bbox: [" << x_center << ", " << y_center << ", " << width << ", " << height << "]" << std::endl;

    // Calculate the aspect ratio scaling factors
    float r_w = input_width_ / (imgSz.width * 1.0);
    float r_h = input_height_ / (imgSz.height * 1.0);

    std::cout << "Scaling factors: r_w = " << r_w << ", r_h = " << r_h << std::endl;

    // Initialize top-left and bottom-right coordinates
    int l, r, t, b;

    if (r_h > r_w) {
        float scale = r_w;
        float pad = (input_height_ - r_w * imgSz.height) / 2;

        std::cout << "Pad (r_h > r_w): " << pad << std::endl;

        l = static_cast<int>((x_center - width / 2.f) / scale);
        r = static_cast<int>((x_center + width / 2.f) / scale);
        t = static_cast<int>((y_center - height / 2.f - pad) / scale);
        b = static_cast<int>((y_center + height / 2.f - pad) / scale);
    } else {
        float scale = r_h;
        float pad = (input_width_ - r_h * imgSz.width) / 2;

        std::cout << "Pad (r_w >= r_h): " << pad << std::endl;

        l = static_cast<int>((x_center - width / 2.f - pad) / scale);
        r = static_cast<int>((x_center + width / 2.f - pad) / scale);
        t = static_cast<int>((y_center - height / 2.f) / scale);
        b = static_cast<int>((y_center + height / 2.f) / scale);
    }

    std::cout << "Bounding box before bounds check: [" << l << ", " << t << ", " << r << ", " << b << "]" << std::endl;

    // Ensure coordinates are within image bounds
    l = std::max(0, l);
    t = std::max(0, t);
    r = std::min(imgSz.width - 1, r);
    b = std::min(imgSz.height - 1, b);

    std::cout << "Bounding box after bounds check: [" << l << ", " << t << ", " << r << ", " << b << "]" << std::endl;

    // Return the cv::Rect object
    return cv::Rect(l, t, r - l, b - t);
}
...
const std::vector<std::vector<int64_t>>& infer_shapes) 
{
    ...
    if(infer_shape[2]  < infer_shape[1])
    {
        ...
        {
            float clsConf = it[4];
            //std::cout << "clsf bigger :  "  <<clsConf<< std::endl;
            if (clsConf > confThreshold)
            {
                //std::cout << "clsf bigger "  << std::endl;
                auto[objConf, classId] = Yolo::getBestClassInfo(it, numClasses);
                std::cout << Yolo::get_rect(frame_size, std::vector<float>(it, it + 4)) << std::endl;
    ...
    std::cout << "post proccess " << diff2 << " ms" << std::endl;
        ...
        for (int i = 0; i < infer_shape[2]; i++) {
            ...
            //std::cout << "bboxesPtr :  "  <<bboxesPtr<< std::endl;
